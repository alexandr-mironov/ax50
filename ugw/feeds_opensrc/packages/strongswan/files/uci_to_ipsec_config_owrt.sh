...
SetIptableRules ()
{
  ...
  config_get dst			"$1" dst
	...
	iptables -t filter -I FORWARD 1 -p tcp --tcp-flags SYN,RST SYN -j TCPMSS --set-mss 1374
}
...
RemoveIptableRules ()
{
  ...
  config_get dst                        "$1" dst
	...
	iptables -t filter -D FORWARD -p tcp --tcp-flags SYN,RST SYN -j TCPMSS --set-mss 1374 2>/dev/null
}
