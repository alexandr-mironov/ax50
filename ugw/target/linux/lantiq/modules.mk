...
define KernelPackage/usb-oc
  TITLE:=USB OC protection on GRX5xx
  SUBMENU:=$(USB_MENU)
  DEPENDS+=+kmod-usb-storage
  KCONFIG:=CONFIG_USB_DWC3_LTQ_OCP=m
  FILES:=$(LINUX_DIR)/drivers/usb/dwc3/ltq-usb-oc.ko
  AUTOLOAD:=$(call AutoLoad,50,usb-oc)
endef

define KernelPackage/usb-oc/description
  Kernel support USB OCP on GRX350 and GRX5xx
endef

$(eval $(call KernelPackage,usb-oc))

define KernelPackage/usb-ltq-dwc2
	TITLE:=LTQ DWC2 support
	SUBMENU:=$(USB_MENU)
	DEPENDS+=+kmod-usb-storage
	KCONFIG:= \
		CONFIG_USB_SUPPORT=y \
		CONFIG_USB_LTQ_DWC2=m \
		CONFIG_USB_DWC2_COMMON_PORT=m
	FILES:= \
		$(LINUX_DIR)/drivers/usb/ltq_dwc2/dwc_otg.$(LINUX_KMOD_SUFFIX) \
		$(LINUX_DIR)/drivers/usb/ltq_dwc2/dwc_common_port/dwc_common_port_lib.$(LINUX_KMOD_SUFFIX)
endef

define KernelPackage/usb-ltq-dwc2/description
	Kernel support LTQ DWC2 for XWAY
endef

$(eval $(call KernelPackage,usb-ltq-dwc2))

define KernelPackage/usb-ltq-dwc2-oc
  TITLE:=USB OC protection on GRX3xx
  SUBMENU:=$(USB_MENU)
  DEPENDS+=+kmod-usb-ltq-dwc2
  KCONFIG:=CONFIG_USB_DWC2_LTQ_OCP=m
  FILES:=$(LINUX_DIR)/drivers/usb/ltq_dwc2/ltq_dwc2_oc.$(LINUX_KMOD_SUFFIX)
endef

define KernelPackage/usb-ltq-dwc2-oc/description
  Kernel support USB OCP on GRX330 and GRX3xx
endef

$(eval $(call KernelPackage,usb-ltq-dwc2-oc))
...
define KernelPackage/gen6-emu
  TITLE:=Enable GEN6 Emulator Config
  SUBMENU:=Lantiq
  KCONFIG:= \
	CONFIG_USE_GEN6_EMULATOR=y
endef

define KernelPackage/gen6-emu/description
  Enable GEN6 Emulator Config
endef

$(eval $(call KernelPackage,gen6-emu))
...
define KernelPackage/vrx318-dp-mod
  ...
  AUTOLOAD:=$(call AutoLoad,42,vrx318_tc)
endef
...
define KernelPackage/vrx518-dp-mod
  ...
  KCONFIG:= \
	CONFIG_VRX518_TC=m
  AUTOLOAD:=$(call AutoLoad,42,vrx518_tc)
endef
...
define KernelPackage/lantiq_directconnect_support
 ...
 AUTOLOAD:=$(call AutoLoad,42,ltq_directconnect_datapath)
endef
...
define KernelPackage/host-os
  SUBMENU:=Lantiq
  TITLE:=Enable Host os
  KCONFIG:=CONFIG_HW_VIRT_PRI_OS=y
endef

define KernelPackage/host-os/description
 Kernel support for host-os
endef

$(eval $(call KernelPackage,host-os))

define KernelPackage/bootcore-serialon
  SUBMENU:=Lantiq
  TITLE:=Bootcore serial Turn on
  DEPENDS:=@TARGET_lantiq_xrx500_4kec
  KCONFIG:=CONFIG_TTY=y \
           CONFIG_SERIAL_GRX500_BOOTCORE_CONSOLE=y
endef

define KernelPackage/bootcore-serialon/description
  Enables the bootcore serial port.
endef

$(eval $(call KernelPackage,bootcore-serialon))
