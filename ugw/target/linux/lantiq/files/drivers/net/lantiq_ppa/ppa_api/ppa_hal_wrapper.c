...
uint32_t ppa_hsel_set_qos_queue_threshold(PPA_QOS_QUEUE_THRESHOLD_CFG *entry, uint32_t flag, uint32_t hal_id)
{
	if (!ppa_drv_hal_hook[hal_id])
		return PPA_FAILURE;

	return ppa_drv_hal_hook[hal_id](PPA_GENERIC_HAL_SET_QOS_QUEUE_THRESHOLD_CFG, (void *)entry, flag);
}
EXPORT_SYMBOL(ppa_hsel_set_qos_queue_threshold);
