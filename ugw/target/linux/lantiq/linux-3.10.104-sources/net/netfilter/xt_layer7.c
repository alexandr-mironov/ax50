...
#endif
{
	...
	if(!info->pkt && (total_acct_packets(master_conntrack) > num_packets ||
	   master_conntrack->layer7.app_proto)) {
		...
		spin_unlock_bh(&l7_lock);
		/*reset the flag to start acceleration*/
		ppa_hook_session_accel_fn(master_conntrack,1);
	...
	if(total_acct_packets(master_conntrack) == 1){
		/*On receiving first the packet stop acceleartion*/
		ppa_hook_session_accel_fn(master_conntrack,0);
	} 
