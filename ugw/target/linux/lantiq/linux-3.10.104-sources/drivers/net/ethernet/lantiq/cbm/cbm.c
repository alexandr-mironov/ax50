	...
	case DP_F_FAST_WLAN:
		...
		if ((!test_ptr) && (!second_if)) {
			...
			cbm_populate_entry(local_entry, &p->phys_port,
					   cbm_port[i], p->flags,
					   &p->pmac_found);
			qid = dqm_port_info[cbm_port[i]].def_qid;
			local_entry->qids[0] = qid;
			local_entry->tmu_res.q_id = qid;
			local_entry->tmu_res.sch_lvl = 1;
...
ALLOC_OK:
	...
	if (num && res) {
		if (dqm_port_info[res[0].tmu_port].sch_lvl == 1) {
			if (!second_if) {
				pr_info("disabling q %d port %d sch_id %d\n", res[0].q_id,
							    res[0].tmu_port, res[0].sch_id[0]);
		...
		cbm_qidt_set(&qidt_set, &qidt_mask, res[0].q_id);
