...
void tmu_proc_eqt_print_hitq(struct seq_file *s)
{
	uint32_t qdc[4], enq_c, deq_c;
	struct tmu_equeue_link qlink;
	struct tmu_equeue_drop_params thx;
	uint32_t qocc, wq, qrth, qavg, index;
	struct tmu_egress_port_thr epth;
	char flag_buf[20];
	char *flag_s;
	seq_printf
	(s, "%15s %10s %10s %10s (%10s %10s %10s %10s) %10s %10s %10s\n",
		"Queue MIB    ID", "enq", "deq", "drop",
		"No-Color", "Green", "Yellow", "Red",
		"qocc", "qavg", "  DMA  Flag");
	for (index = 0; index < EGRESS_QUEUE_ID_MAX; ++index){
		...
		flag_buf[0] = 0;
		flag_s =
			get_dma_flags_str(qlink.epn,
						flag_buf, sizeof(flag_buf));
		if ((enq_c || deq_c || (qdc[0] + qdc[1] + qdc[2] + qdc[3]))
			|| qocc || qavg) {
			seq_printf
			(s, "%15d %10u %10u %10u (%10u %10u %10u %10u) %10u %10u %10s\n",
				index, enq_c, deq_c, (qdc[0] + qdc[1] + qdc[2]
				+ qdc[3]), qdc[0], qdc[1], qdc[2], qdc[3], qocc,
			   ...
			   loff_t *ppos)
{
	...
	if (num < 2) {
		...
		TMU_PRINT("Note: if queue-id is -1, it means all queues\nBut if you need the output to be displayed in telnet session, then use cat /proc/tmu/hitq\n");
		TMU_PRINT
