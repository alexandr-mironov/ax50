...
start() {
	...
	if [ "$platform" = "GRX350" -o "$platform" = "GRX550" -o "$platform" = "IRE220" ]; then
	...
	# Load USB OCP driver
	if [ -f /lib/modules/*/ltq-usb-oc.ko ]; then
		echo "Loading USB OCP driver"
		insmod /lib/modules/*/ltq-usb-oc.ko
	fi

	# Load USB DWC2 driver
	if [ "`cat /proc/device-tree/fpi*/usb@*/status | grep -c okay`" -gt 0 ]; then
		if [ -f /lib/modules/*/dwc_otg.ko ]; then
			echo "Loading DWC2 USB driver"
			insmod /lib/modules/*/usb-storage.ko
			insmod /lib/modules/*/dwc_common_port_lib.ko
			insmod /lib/modules/*/dwc_otg.ko
			insmod /lib/modules/*/usblp.ko
		fi
	fi

	# Load DWC2 USB OCP driver
        if [ -f /lib/modules/*/ltq_dwc2_oc.ko ]; then
                echo "Loading DWC2 USB OCP driver"
                insmod /lib/modules/*/ltq_dwc2_oc.ko
        fi
