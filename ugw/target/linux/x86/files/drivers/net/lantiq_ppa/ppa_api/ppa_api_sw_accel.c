...
#define IsPPTPSession(flags)        ( (flags) & SESSION_FLAG2_PPTP ) 
...
static int32_t pptp_gre_header_len = 12;
#endif
...
#if defined(CONFIG_PPTP_MODULE) || defined(CONFIG_PPTP)
   } else if( IsPPTPSession(p_item->flag2) ) { 
     
     /* Handle PPTP Tunneled sessions */ 
     if(  IsLanSession( p_item->flags) ) {
	if(ppa_get_pptp_hdrlen_fn)
		headerlength = PPTP_HDR_LEN;
		
     }
     else {
	if(ppa_get_pptp_hdrlen_fn)
		headerlength = ppa_get_pptp_hdrlen_fn(p_item->rx_if,p_item->chan_private);
     }
	
     headerlength += sizeof(struct iphdr) + 4; 
...
#if defined(CONFIG_PPTP_MODULE) || defined(CONFIG_PPTP)
static uint32_t ppa_form_pptp_tunnel(const struct session_list_item* p_item,
		uint8_t* hdr ,unsigned isIPv6)
{
	...
	// extracting  mac header and IP header
	if(ppa_pptp_get_outer_src_dst_addr_fn)
		base_dev = ppa_pptp_get_outer_src_dst_addr_fn(p_item->tx_if,p_item->chan_private,&outer_srcip,&outer_dstip,&rt,mac,NULL);

	ppa_get_netif_hwaddr(base_dev, srcmac, 1);
	ppa_memcpy(hdr+ETH_ALEN ,srcmac, PPA_ETH_ALEN);
	ppa_memcpy(hdr, mac, PPA_ETH_ALEN);
        ppa_memcpy(p_item->dst_mac,mac,PPA_ETH_ALEN);	
        ppa_memcpy(p_item->src_mac,srcmac,PPA_ETH_ALEN);	
	hdr += ETH_ALEN*2;
	len =ETH_HLEN;
	...
	*((uint16_t*)(hdr)) = htons(proto_type);
	...
	iph.ihl    = 5;
	iph.ttl    = 64 ;
	iph.saddr  = outer_srcip;
	iph.daddr  = outer_dstip;
	...
	// adding GRE header to templet buffer
	if(ppa_form_pptp_hdr_fn)
		pptphdrlen = ppa_form_pptp_hdr_fn(p_item->tx_if,p_item->chan_private,hdr);
	// adding PPP header to template buffer
	len +=pptphdrlen;
	*(hdr+ pptphdrlen) = 0xff;
	*(hdr +pptphdrlen+ 1) = 0x03;

	if(isIPv6)
		*(uint16_t*)(hdr + pptphdrlen + 2) = 0x0057;
	else
		*(uint16_t*)(hdr + pptphdrlen + 2) = 0x0021;

	len+=PPP_LEN; //ip+gre+ppp
	return len;
}
#endif
...
int32_t sw_update_session(PPA_BUF *skb, struct session_list_item *p_item, struct netif_info *tx_ifinfo)
{
  ...
  if(IsGreSession(p_item) && !IsPPTPSession(p_item->flag2)) {
     return PPA_SUCCESS;
  }
  ...
  if(p_item->sah == NULL) {
    ...
    else if( IsTunneledSession(p_item->flags)||IsPPTPSession(p_item->flag2) ) {
      swaHdr.tot_hdr_len = tlen;
#if defined(CONFIG_PPTP_MODULE) || defined(CONFIG_PPTP)
      if( IsPPTPSession(p_item->flag2) ) { 
        swaHdr.type = SW_ACC_TYPE_PPTP;
        swaHdr.tot_hdr_len = tlen;
        if( IsLanSession(p_item->flags) ) {
		swaHdr.network_offset = ETH_HLEN;
		if( IsValidVlanIns(p_item->flags) ) 
          		swaHdr.network_offset += 4;
		if (IsPppoeSession(p_item->flags))
          		swaHdr.network_offset +=  8;
          //swaHdr.network_offset = tlen ;
	...
	if( ! (IsTunneledSession( p_item->flags)) && (!IsPPTPSession(p_item->flag2)) ) {
		...
		if( IsLanSession(p_item->flags) ) {  
		    ...
		    ppa_memcpy(p_swaHdr->hdr + p_swaHdr->network_offset + 16, &p_item->nat_ip.ip, 4);  
      		...
      		if( IsLanSession(p_item->flags) ) {
			  tlen  = ppa_form_pptp_tunnel(p_item,p_swaHdr->hdr,isIPv6); 
			  swaHdr.tot_hdr_len  = tlen;
		}
		else
		{
			//uint8_t* hdr;
			hdr = (uint8_t*)p_swaHdr->hdr;
			ppa_get_netif_hwaddr(p_item->tx_if, srcmac, 1);
			ppa_memcpy(hdr+ETH_ALEN ,srcmac, PPA_ETH_ALEN);
		        ppa_memcpy(p_item->src_mac,srcmac,PPA_ETH_ALEN);
			hdr +=ETH_ALEN*2;
			proto_type = ETH_P_IP;
...
//skip the telnet, http & https sessions; tcp port 23, 80, 8080, 8000, 8001, 443
	if (p_item->src_port==23 || p_item->dst_port==23 ||
		p_item->src_port==80 || p_item->dst_port==80 ||
		p_item->src_port==8080 || p_item->dst_port==8080 ||
		p_item->src_port==8000 || p_item->dst_port==8000 ||
		p_item->src_port==8001 || p_item->dst_port==8001 ||
		p_item->src_port==443 || p_item->dst_port==443) {
                              ...
                              unsigned char data_offset)
{
  ...
  switch(iph->protocol)
  {
    ...
    case IP_PROTO_GRE :  
    {
		if (ppa_check_pptp_ack_flag_fn)
			pptp_gre_header_len = ppa_check_pptp_ack_flag_fn(data + IPV4_HDR_LEN);
		data_offset +=	pptp_gre_header_len+IPV4_HDR_LEN ;
        	pFlagHdr->is_inner_ipv4 = 1;
#if defined(CONFIG_LTQ_PPA_GRX500) && CONFIG_LTQ_PPA_GRX500
		pFlagHdr->is_gre = 1;
		pFlagHdr->offset[PARSER_IPV4_INNER_OFFSET_IDX] = data_offset;
#endif
		iph = (struct iphdr*)data + data_offset - ETH_HLEN;
		switch(iph->protocol)
  		{
    			case IPPROTO_TCP:
			{
      				tcph = (struct tcphdr *)(data + data_offset + IPV4_HDR_LEN-ETH_HLEN);  
				if(tcph->rst||tcph->fin || (tcph->syn && tcph->ack)) {
#if defined(CONFIG_LTQ_PPA_GRX500) && CONFIG_LTQ_PPA_GRX500
		        		pFlagHdr->is_rt_excep = 1;
#else
			        	pFlagHdr->tcp_rstfin = 1;
...
#if defined(CONFIG_LTQ_PPA_GRX500) && CONFIG_LTQ_PPA_GRX500
  return ( ( flg_hdr->is_udp || flg_hdr->is_tcp || flg_hdr->is_gre ) && !flg_hdr->is_rt_excep );
#else
...
#endif
#if defined(CONFIG_PPTP_MODULE) || defined(CONFIG_PPTP)
static int sw_mod_pptp_skb( PPA_SKBUF *skb, 
                               struct session_list_item *p_item)
{
	int ret=0 ;
	uint8_t mac_hdr[14];
	...
	ret = skb->len+swa->tot_hdr_len;
  	...
  	if( IsLanSession(p_item->flags) ) {
		ppa_memcpy(&org_iph, skb->data, sizeof(org_iph));
		if(ppa_get_pptp_hdrlen_fn)
			headerlength = ppa_get_pptp_hdrlen_fn(p_item->tx_if,p_item->chan_private);

		if((swa->tot_hdr_len - swa->transport_offset - PPP_LEN) != headerlength) {// if no ack then dont copy the ack part
		
			*(swa->hdr + swa->transport_offset + headerlength ) = 0xff;
			*(swa->hdr + swa->transport_offset + headerlength + 1) = 0x03;
			*(uint16_t*)(swa->hdr + swa->transport_offset + headerlength +2 ) = 0x0021;
			skb_push(skb, (swa->tot_hdr_len - 4) );
			ppa_memcpy(skb->data, swa->hdr, (swa->tot_hdr_len - 4));
		...
		if(ppa_update_pptp_hdr_fn)
			ppa_update_pptp_hdr_fn(p_item->tx_if,p_item->chan_private,skb->data+swa->transport_offset,skb->len-swa->transport_offset);
		iph = skb->data + swa->network_offset;
		iph->tot_len =skb->len-swa->network_offset ;
		iph->id = org_iph.id;
	...
	{
		if(ppa_pptp_rcv_fn)
			ppa_pptp_rcv_fn(p_item->rx_if,p_item->chan_private,skb,pptp_gre_header_len);
		skb_push(skb, swa->tot_hdr_len);

  		ppa_memcpy(skb->data, swa->hdr, swa->tot_hdr_len);
		ppa_memcpy(skb->data,p_item->dst_mac,PPA_ETH_ALEN);
...
handle_gre:
  ...
  if (IsSoftwareAccelerable(flg_hdr)) {
	...
	if( (IsL2TPSession(p_item->flags) || 
        	  ! (IsSoftwareAccelerated(p_item->flags)) && !(IsPPTPSession(p_item->flag2)) ) ||
          	((skb->len > p_item->mtu) 
#if defined(CONFIG_LTQ_PPA_LRO) && CONFIG_LTQ_PPA_LRO
