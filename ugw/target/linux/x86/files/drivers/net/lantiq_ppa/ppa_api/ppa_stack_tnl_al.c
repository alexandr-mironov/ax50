...
struct net_device *ppa_get_pptp_dmac( uint8_t *mac,
		...
		uint32_t  daddr)
{
	...
	if( ppa_check_is_ppp_netif(phydev)) {
		...
		pkey = (__be32)daddr;
		neigh = __ipv4_neigh_lookup(phydev, (__force u32)(pkey));
		if(neigh) {
...
EXPORT_SYMBOL(ppa_get_pptp_dmac);
#endif
...
#if defined(CONFIG_PPTP_MODULE) || defined(CONFIG_PPTP)
/*
 * Returns 1 if given netif is pptp tunnel, else returns zero
 */
uint32_t ppa_is_pptp_netif(struct net_device* dev)
{
	struct sock *sk = NULL;
	uint32_t  ret = 0;
	if(ppa_get_pptp_info_fn) {
		sk = ppa_get_pptp_info_fn(dev); 
		if(sk){
...
EXPORT_SYMBOL(ppa_is_pptp_netif);
#endif
/*
		...
		struct sk_buff *skb)
{
	...
	if( ppa_check_is_ppp_netif(phydev)) {
		...
		if ( ppa_check_is_pppoe_netif(phydev) ) {
			...
			ret = ppa_get_ipv4_tnl_dmac(mac, dev, skb); // PPTP dst ac reading
		}
