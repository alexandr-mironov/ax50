...
/* Set skb extmark bit to tell the stack packet is not classified by Flow rule */
    ...
    if (TI_HIL_GET_PP_INFO_FLAGS(ppa_buf) & TI_HIL_PACKET_FLAG_PP_SESSION_BYPASS) {
...
#if defined(CONFIG_PPTP_MODULE) || defined(CONFIG_PPTP)
	&& !(p_item->flag2 & SESSION_FLAG2_PPTP)
#endif
	)
    {
...
#endif
    if ( (rx_ifinfo->flags & NETIF_GRE_TUNNEL) ||(rx_ifinfo->flags & NETIF_PPTP_TUNNEL) ) {
...
#if defined(CONFIG_PPTP_MODULE) || defined(CONFIG_PPTP)
	if(rx_ifinfo->flags & NETIF_PPTP_TUNNEL) { 
		//ppa_netif_lookup(ppa_get_netif_name(base_netif), &rx_ifinfo)
    		ppa_set_PPTPSession(p_item); 
    		p_item->flag2 |= SESSION_FLAG2_ADD_HW_FAIL;  //  No H/W accleration
		hdr_offset += 50;
		if(rx_ifinfo->flags & NETIF_VLAN)
      			hdr_offset += 4; 
		if ( rx_ifinfo->flags & NETIF_PPPOE ) {
		    	hdr_offset += PPPOE_SES_HLEN ; // PPPoE
		}
...
int32_t ppa_update_session_info(PPA_BUF *ppa_buf, struct session_list_item *p_item, uint32_t flags)
{
      ...
      while ( (tx_ifinfo->flags & NETIF_BRIDGE) )
      {
		...
		struct net_bridge *br;
	      ...
	      // updae dst mac 
        	...
        	ppa_get_dst_mac(ppa_buf, NULL, p_item->dst_mac,0);
  ...
  if( (!rx_local && ppa_netif_type(p_item->tx_if) == ARPHRD_SIT) || (!tx_local && ppa_netif_type(p_item->rx_if) == ARPHRD_SIT) ){
...
#endif
#if defined(CONFIG_PPTP_MODULE) || defined(CONFIG_PPTP)
 } else if(( rx_ifinfo && ( (rx_ifinfo->flags & NETIF_PPTP_TUNNEL)))  ) {  // inluded PPTP tunnel check
    PPA_NETIF *base_netif;
    PPA_NETIF *temp_netif;
    ppa_set_PPTPSession(p_item); 
    temp_netif = p_item->rx_if;
    p_item->rx_if = temp_netif;
    if(ppa_get_pptp_info_fn)
    	p_item->chan_private = ppa_get_pptp_info_fn(p_item->rx_if);
    ...
    p_item->rx_if = temp_netif;
#if defined(CONFIG_PPTP_MODULE) || defined(CONFIG_PPTP)
  } else if( tx_ifinfo && (tx_ifinfo->flags & NETIF_PPTP_TUNNEL)) { 
    PPA_NETIF *base_netif;
    PPA_NETIF *temp_netif; 
    ppa_set_PPTPSession(p_item); 
    temp_netif = p_item->tx_if;
    if(p_item->chan_private == NULL)
    {

	if(ppa_get_pptp_info_fn)
    		p_item->chan_private = ppa_get_pptp_info_fn(p_item->tx_if); 
    }
...
int32_t ppa_hw_add_session(struct session_list_item *p_item, uint32_t f_test)
{
    ...
    if( ppa_is_BrSession(p_item) || ppa_is_GreSession(p_item) || ppa_is_PPTPSession(p_item))
      return PPA_FAILURE;
...
#if defined(CONFIG_PPTP_MODULE) || defined(CONFIG_PPTP)
	if((rx_ifinfo->flags & NETIF_PPTP_TUNNEL) == NETIF_PPTP_TUNNEL ) {
		p_item->flag2 |= SESSION_FLAG2_PPTP;
	}
#endif
