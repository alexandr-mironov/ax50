...
_init_local()
{
	...
	BIN_PATH=$(cd `pwd`/../../../build_support/tools/usr/bin && pwd)
...
repo_deinit()
{
	repo &>/dev/null
	[[ $? -eq 0 ]] && {
		echo "UGW repo discovered, Checkout to manifest revisions and delete temp branches, local changes and untracked files"
		repo forall -p -c 'git checkout .; git clean -xf; git checkout $REPO_LREV' &> /dev/null
		repo forall -p -c 'git branch -D temp_"$REPO_RREV"' &>/dev/null
		echo "Done"
	}
}
...
repo_init()
{
	repo &>/dev/null
	[[ $? -eq 0 ]] && {
		echo "UGW repo discovered, save manifest and checkout all detached heads to temp branches"
		repo manifest -o manifest_$(whoami)_$(date +%d-%m-%Y).xml -r
		repo forall -c 'if [ -n "$(git branch | grep "* (no branch)\|* (HEAD detached")" ]; then git checkout -b temp_"$REPO_RREV"; fi' &> /dev/null
		echo "Done"
	}
}
...
custom_changes_to_revision()
{
	[ -h .git/patches ] && {
		...
		guilt new buildsystem/ugw-temperory-from-ugw-prepare.patch -m "Temp_Commit_Donot_Push" -f
		guilt refresh
		#quilt new buildsystem/ugw-temperory-from-ugw-prepare.patch
	}
}
